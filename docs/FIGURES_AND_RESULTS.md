...
| Reference | What it shows |
|-----------|----------------|
| [`figures/all_features_model_comparison.png`](../figures/all_features_model_comparison.png) | **Bar chart**: **mean CV accuracy** for **nine** learners on the **21-feature** superset matrix (4 financial ratios + the full 12-column KAM/firm block + 5 news aggregates), same 45 rows. Extra Trees leads at **82.2%**; read alongside macro-F1, which is **0.68**. Numeric details: [`results/all_features_model_results.json`](../results/all_features_model_results.json). |
